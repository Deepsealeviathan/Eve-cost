...
└── Reactions/                 # 反应配方
    ├── 混合聚合物/            # 矿物+气云+燃料块反应产物
    ├── 中间产物/              # 卫星原料+燃料块反应产物(纯产物)
    ├── 合成物/                # 中间产物+燃料块反应产物
    ├── 分子锻造材料/          # 气云+挖坟材料反应产物(神经链接/新富勒烯)
    └── 燃料块/                # 反应消耗材料(基础材料,按市场价计算)
└── Components/                # 制造组件配方
    ├── 旗舰基础组件/          # 基础旗舰组件(矿物+合成物等)
    ├── 旗舰高级组件/          # 高级旗舰组件(合成物+分子锻造材料等)
    ├── T2制造组件/            # T2 组件(合成物+分子锻造材料等)
    └── 基础材料/              # 不可合成的组件输入(深渊材料/旗舰组件材料等,按市场价)
```
...
Web 页面支持:单个/批量成本查询、物品目录浏览(分类 + 搜索)、一键更新数据库(调用 `update_market.py`),以及组件制造计算器(访问 `/components`,目录选组件、数量加减、建筑加成减免)。
